// ---------------------------------------------------------------------------
// Base
// ---------------------------------------------------------------------------

export const NEOPETS_BASE_URL = 'https://www.neopets.com';

// ---------------------------------------------------------------------------
// Page URLs
// ---------------------------------------------------------------------------

export const SAFETY_DEPOSIT_BOX_URL = `${NEOPETS_BASE_URL}/safetydeposit.phtml`;
export const SHOP_WIZARD_URL        = `${NEOPETS_BASE_URL}/shops/wizard.phtml`;
export const SUPER_SHOP_WIZARD_URL = `${NEOPETS_BASE_URL}/np-templates/views/shops/ssw/ssw_query.php`
export const FOOD_CLUB_URL          = `${NEOPETS_BASE_URL}/medieval/foodclub.phtml`;

// ---------------------------------------------------------------------------
// External API URLs
// ---------------------------------------------------------------------------

export const HISTORICAL_PRICE_API_BASE = 'https://raw.githubusercontent.com/Unovamata/AutoBuyerPlus/refs/heads/main/Autobuyer/DB/database.json';

// ---------------------------------------------------------------------------
// Storage keys
// ---------------------------------------------------------------------------

export const BET_HISTORY_STORAGE_KEY = 'neopets:betForecast:history';
export const HISTORICAL_PRICE_CACHE_KEY = 'neopets:restocker:priceSnapshot';
export const LEGACY_SHOP_WIZARD_AJAX_URL = 'https://www.neopets.com/np-templates/ajax/wizard.php';
