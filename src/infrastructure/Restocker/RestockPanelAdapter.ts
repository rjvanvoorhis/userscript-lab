...
export class RestockPanelAdapter implements IRestockPanel {
  ...
  private bestItemEl: HTMLElement | null = null;

  mount(shops: Record<string, string>, initial: RestockConfig): void {
    const autobuyEl = el("input", {
      type: "checkbox",
      id: "rp-autobuy",
      checked: initial.autobuyEnabled,
    });
    const autorefreshEl = el("input", {
      type: "checkbox",
      id: "rp-autorefresh",
      checked: initial.autorefreshEnabled,
    });
    const frequencyEl = el("input", {
      type: "number",
      id: "rp-frequency",
      value: String(initial.refreshFrequencyMs),
      min: "1000",
      style: "width:70px",
    });
    const marginEl = el("input", {
      type: "number",
      id: "rp-margin",
      value: String(initial.minProfitMargin.amount),
      min: "0",
      style: "width:70px",
    });
    ...
    for (const [id, name] of Object.entries(shops)) {
      shopEl.appendChild(
        el("option", {
          value: id,
          textContent: name,
          selected: id === initial.shopId,
        }),
      );
    }

    const table = el("table", {
      style: "margin-top:8px;border-spacing:4px 6px",
    });
    table.appendChild(labelledRow("Autobuy", "rp-autobuy", autobuyEl));
    table.appendChild(
      labelledRow("Autorefresh", "rp-autorefresh", autorefreshEl),
    );
    table.appendChild(
      labelledRow("Frequency (ms)", "rp-frequency", frequencyEl),
    );
    ...
    panel.appendChild(this.bestItemEl);
    document.body.appendChild(panel);

    const emit = () => {
      const autobuyEnabled = autobuyEl.checked;
      const autorefreshEnabled = autorefreshEl.checked;
      const refreshFrequencyMs = Math.max(
        1000,
        Number.parseInt(frequencyEl.value, 10) || 5000,
      );
      ...
      try {
        minProfitMargin = NeoPoint.from(
          Number.isNaN(marginRaw) ? 0 : marginRaw,
        );
      } catch {
        return;
      }
